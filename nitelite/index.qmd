...
Below is a table summarizing the model results: 

![Table Summary of ARDL and VECM Results](fig/tablesummary.png){#fig-11}

What is immediately apparent from the results table is that the model error is quite low, with the ARDL model incorporating quarterly dummies posting both MAE and RMSE of 0.005. It must be noted, however, that the results come with some caveats. The ARDL model results are very promising, but the model's forecasting capabilities are limited only to one step ahead, or one quarter within this context. Because the model contains lagged values of both the independent and dependent variables, it cannot forecast multiple steps ahead without first forecasting the independent variable separately (in an exogenous variable forecast) or assuming the independent variable stays constant, and neither option is ideal. To properly use the ARDL model, updated nighttime lights data is required. 

Most VECM models are also posting a relatively low error value, but this may be due in large part to the fact that much of the VECM fit is driven by the previous GDP values. Unfortunately, the nighttime lights variable is not significant at the 0.05 level in most of the models. The model is experiencing a large amount of assistance in its predictions based on prior GDP data, instead of nighttime lights data.
